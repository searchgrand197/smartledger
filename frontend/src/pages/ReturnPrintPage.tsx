...
import ReturnPrintLayout from "@/components/returns/ReturnPrintLayout";
...
export default function ReturnPrintPage({ portal = false }: { portal?: boolean }) {
  ...
  useEffect(() => {
    ...
    const apiCall = portal ? portalApi.returnPrintData(returnId) : returnsApi.printData(returnId);
    apiCall
      .then((res) => setData(res.data as ReturnPrintData))
      .catch((err: unknown) => {
        const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
        setError(msg || "Could not load return receipt");
      });
  }, [id, portal]);
