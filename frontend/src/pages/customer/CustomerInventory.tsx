...
export default function CustomerInventory() {
  ...
  const [editing, setEditing] = useState<PortalProduct | null>(null);
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["customer-portal-products", search],
    queryFn: () => portalApi.products({ search: search || undefined, all: true }).then((r) => r.data as PortalProduct[]),
  });

  const { data: categoryRows } = useQuery({
    queryKey: ["customer-portal-categories"],
    queryFn: () => portalApi.categories().then((r) => r.data || []),
    enabled: formOpen,
  });
  ...
  const openEdit = (p: PortalProduct) => {
    setEditing(p);
    const wholesale = Number(p.wholesale_rate);
    const retail = p.retail_rate && p.retail_rate > 0 ? Number(p.retail_rate) : 0;
  ...
  const resolveCategoryId = async (): Promise<number | null> => {
    ...
    const { data } = await portalApi.createCategory(name);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await portalApi.updateProduct(editing.id, payload);
  ...
  const handleDeleteProduct = async (id: number, name: string) => {
    ...
    try {
      await portalApi.deleteProduct(id);
  ...
  const ProductThumb = ({ product }: { product: PortalProduct }) => {
  ...
  const columns: TableColumn<PortalProduct>[] = [
    {
      id: "name",
      label: "Item",
      minWidth: 220,
    ...
    {
      id: "retail_rate",
      label: "Retail (Quick Sale)",
      align: "right",
      render: (r) => (
        <Typography fontWeight={600} fontSize="0.875rem" color="primary.dark">
          {formatCurrency(r.retail_rate ?? r.your_rate)}
        </Typography>
      ),
    },
    {
      ...
      render: (r) => (
        <Typography fontWeight={600} fontSize="0.875rem">
          {formatCurrency(r.wholesale_rate)}
        </Typography>
      ),
    },
  ...
  const formFields = (
    <Stack spacing={2.5} sx={{ pt: 0.5 }}>
      ...
      <TextField
        ...
        inputProps={{ list: "customer-category-suggestions" }}
      />
      <datalist id="customer-category-suggestions">
        {categories.map((c) => (
  ...
  return (
    <PageShell>
      <PageHeader
        title="Products"
        subtitle="Retail prices for walk-in sales — wholesale rates are for party billing only"
      />
